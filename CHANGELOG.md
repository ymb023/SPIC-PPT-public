...
## [4.12.0] — 2026-06-26 · 两个装配版式：多图横排 + 双栏居中（旧 PPTX 改造实测产出）

用真实长 PPTX（152 页太阳能讲课稿）做"保骨换皮"测试，撞出两个对齐难题、手调三轮没凑对，根因是缺专用版式。本版焊进 CSS 根治。

### Added（components.css，第 18 个版式 + 1 个 body-grid 变体）
- **18 多图横排 `.charts-row`**：N 张比例不同的数据图横排，等高图槽（每槽 `flex:1`→底对齐）+ `object-fit:contain`（不裁切不变形、框内居中）+ 整体 `flex:1`（配 `.compact` 不溢出）。支持 `.cols-2/3/4`。**解决手调三轮的死结**：三图横排先 +79px 溢出→压小（图太小）→放大又参差，这套 CSS 一次过闸、底对齐。
- **`.body-grid.v-center`**：一栏带背景卡撑满、另一栏内容不满时，默认 `align-items:stretch` 会暴露视觉不齐（实测 P16 公式卡 356px vs 表格 196px 差 159px）。`.v-center` 改两栏内容垂直居中，高度差变上下对称留白。

### Changed
- `SKILL.md` 版式表 17→18；双栏对齐速查加 `.v-center` 规则、新增"多图横排"行。
- `template/_layout-demo.html` 加第 8 页 charts-row 范例（自包含占位块，进 eval_suite 回归）。
- `components.css` 顶注 4.11.4 → 4.12.0。

### 质量纪律
- 只用现有 `:root` 变量、新增类不改旧组件（符合"新版式沿用清单"+"Don't modernize"）。
- 自测自审（用户授权）：建含两版式的夹具→过溢出+几何闸门（**charts-row 一次过，手调三轮没做到**）→渲染 PNG 自审三图底对齐、双栏齐平→焊进 demo→_layout-demo 7 页全过闸。
- **实测驱动**：两个版式都不是凭空设计，是真改 152 页 PPTX 撞出来的；流程类三条增强（保骨换皮工作流/视觉映射表/PPTX 提取脚本）留下次。
