...
"""几何护栏：检测装配好的 PPT HTML 里两类可量化的版式违规。
  ...
  1. 字号 < 8pt（"真看不清"的绝对红线——缩字号缩过头/手误写了 6pt）。
     注意不是 14pt：theme.css 字号分级（正文14/小正文13/标签12/来源11 全合法），
     靠 class 名猜"是不是正文"必然误报沦为噪音，故只守 8pt 这条谁都不争议的底线。
     标签/说明小字的合规交回人眼审美。详见下方 MIN_PT 注释。
