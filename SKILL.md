...
- **旧页复用决策**（当用户提供"已成型的旧 PPT 页"要并入本 deck 时，必做；见下方专节）
- 建议略去的内容
...
#### 复用旧 PPT 页：先诊断、默认保留（换皮不换骨）

当用户提供"已成型的旧 PPT 页"（截图/导出页/旧 deck）要并入本 deck，**不要默认整页重绘**——旧页往往是精心排过的，它的图文布局藏着设计判断（为什么上文下图、为什么这里放地图）。先理解，再决定动多少。

**铁律：**
1. **默认理解+保留，不默认重绘。** 先读懂旧页的布局智慧，能留就留。重绘是最后手段，不是第一反应。
2. **换皮不换骨。** 皮 = VI 视觉层（配色/字体/LOGO/飘带/背景），必须换成 SPIC 白底 hairline 风格以与全 deck 一致；骨 = 旧页的图文结构与布局，非必要不动。
3. **诊断先行。** 复用旧页必须先出"旧页诊断"给用户审定，**不得跳过直接重绘**（同结构抽取"先审后做"）。

**每个要复用的旧页，在结构抽取里写一份诊断：**
```
旧页 P_X 诊断：
- 原布局：上文下图 / 左文右图 / 图表+说明 / 纯文字 / ...
- 排版评价：合理（精心排过，值得保留）/ 一般 / 乱（不符投屏或与叙事不搭）
- 图的性质：数据图（可重画成 SPIC 原生）/ 真实界面·地图（截图保留）/ 装饰（可弃）
- 建议动作（三选一）：
    ① 最小换皮：只改色/字/LOGO/背景，布局原样不动（排版本就合理时首选）
    ② 保骨换图：布局留，数据图用 CSS/SVG 重画成 SPIC 原生，截图类保留
    ③ 真重绘：排版本身不行/不符投屏，才用 skill 标准版式重做
- 理由：[为什么选这一档]
```

**装配时**：按用户审定的档位执行——最小换皮/保骨换图保留原"上文下图"等布局骨架（用 `body-grid` 双栏或全宽图承载），只有"真重绘"才套标准版式。图的"重画 vs 截图保留"沿用上方图片决策的判据（数据图重画、真实界面/地图保留）。
